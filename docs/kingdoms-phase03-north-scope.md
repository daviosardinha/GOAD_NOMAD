...
| RBCD Stage 2 | `WS01$` relay reported delegation write success for `PHASE03RBCD$`; descriptor verification pending | MUTATION OBSERVED |
...
1. Run the Stage-2 RBCD descriptor verifier.
2. Prove the S4U consequence using the controlled `PHASE03RBCD$` account.
3. Restore the captured baseline and confirm `PHASE03RBCD$` is removed.
4. Continue to the remaining relay/post-relay families.
