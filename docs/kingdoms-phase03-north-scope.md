...
3. Technique-specific **runtime proof** preserves already-proven Eddard→CASTELBLACK administrative SMB relay + SAM extraction, Robb/Eddard capture behavior, MSSQL sql_svc callback and supported RPC callback(s), then adds the remaining scoped ADIDNS, .lnk/.url victim interaction, WebDAV client scenario, LDAP read-only relay and reversible RBCD. Deterministic mitm6 steering and automatic WPAD PAC retrieval are already proven in NORTH. Optional Shadow Credentials only after explicit ACL preflight and reset proof.
