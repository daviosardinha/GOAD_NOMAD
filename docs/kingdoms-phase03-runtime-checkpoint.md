...
- `NORTH\ROBB.STARK` produced retained interactive SMB sessions on `127.0.0.1:11000+`; share enumeration worked, but opening/listing `C# Kingdoms — Phase 03 Runtime Checkpoint
...
RBCD is therefore closed end-to-end: **preflight -> relay -> mutation -> S4U consequence -> exact rollback**.
 ...
 was denied.
- `NORTH\EDDARD.STARK` produced retained interactive SMB sessions on `127.0.0.1:11000+`; an Eddard session was connected with netcat, `C# Kingdoms — Phase 03 Runtime Checkpoint
...
RBCD is therefore closed end-to-end: **preflight -> relay -> mutation -> S4U consequence -> exact rollback**.
 ...
 was selected, and the root directory was listed successfully.
