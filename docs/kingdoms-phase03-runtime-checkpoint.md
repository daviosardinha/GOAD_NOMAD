...
Shadow Credentials against `WS01# Kingdoms — Phase 03 Runtime Checkpoint
...
DPAPI is therefore closed end-to-end: **administrative relay -> retained SOCKS session -> native DPAPI artifact acquisition -> masterkey GUID correlation -> DPAPI_SYSTEM key recovery -> masterkey decryption -> Credential Manager decryption -> local secret-bearing artifact cleanup**.
 is **PROVEN** and exactly rolled back.

Preflight established:

- target `WS01# Kingdoms — Phase 03 Runtime Checkpoint
...
- WINTERFELL reported one usable KDC certificate through `certutil -dcinfo verify`;
...
- the deterministic WS01 LocalSystem HTTP callback authenticated as `NORTH\WS01# Kingdoms — Phase 03 Runtime Checkpoint
...
- ntlmrelayx generated the temporary certificate/private-key material;
- ntlmrelayx updated `WS01# Kingdoms — Phase 03 Runtime Checkpoint
...
- Certipy authenticated as `north.sevenkingdoms.local/WS01# Kingdoms — Phase 03 Runtime Checkpoint
...
- `WS01# Kingdoms — Phase 03 Runtime Checkpoint
...
DPAPI is therefore closed end-to-end: **administrative relay -> retained SOCKS session -> native DPAPI artifact acquisition -> masterkey GUID correlation -> DPAPI_SYSTEM key recovery -> masterkey decryption -> Credential Manager decryption -> local secret-bearing artifact cleanup**.
 KeyCredentialLink was restored to the exact captured baseline;
