...
class Phase03RBCDSourceTests(unittest.TestCase):
    ...
    def test_attribute_acl_scope_and_guarded_reset(self):
        ...
        self.assertIn("DaclRemovalPreviewAceDelta", source)
        self.assertIn("Get-AceMultisetDelta $original $copy", source)
        self.assertIn("DaclRemovalPreviewRemainingFixtureAces", source)
        ...
        self.assertIn("$restored.RemoveAccessRuleSpecific($restoredMatching[0])", source)
        self.assertNotIn("$acl.RemoveAccessRuleSpecific($rule)", source)
        self.assertIn("function Get-RawAceSignatures", source)
        self.assertIn("function Get-DaclControlMask", source)
        self.assertIn("function Assert-OriginalDaclEquivalent", source)
        self.assertIn("Get-AceMultisetDelta $reference $candidate -Raw", source)
        self.assertIn("$preflight = Assert-OriginalDaclEquivalent $original $restored", source)
        self.assertIn("$postCheck = Assert-OriginalDaclEquivalent $original $observed", source)
        self.assertIn("DaclRemovalPreviewRawAceDelta", source)
        self.assertIn("EquivalentRawAcesAndDaclFlags", source)
        self.assertIn("OriginalRawAcesAndDaclFlagsVerified", source)
