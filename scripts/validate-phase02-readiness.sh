...
if [[ ${#NXC[@]} -gt 0 ]]; then
  ...
  # WS01 is intentionally visible to the Phase 00 SMB service map. Valid NORTH
  # identities must reach and authenticate to SMB, while low-privilege users
  # must not gain administrative-share file access.
  for user in "${USERS[@]}"; do
    log="$EVIDENCE/smb-ws01-$user.log"
    run_nxc "$log" smb "$WS01" -d "$DOMAIN_FQDN" -u "$user" -p "${PASSWD[$user]}" --shares >/dev/null 2>&1
    if auth_ok "$log"; then
      pass "WS01 accepts NORTH\\$user over SMB"
    else
      fail "WS01 did not accept NORTH\\$user over SMB"
      continue
    fi

    for share in 'ADMINif [[ ${#NXC[@]} -gt 0 ]]; then
...
exit 1; do
      if share_has "$log" "$share" READ || share_has "$log" "$share" WRITE; then
        fail "WS01 $user unexpectedly has administrative-share access on $share"
      else
        pass "WS01 $user has no unexpected administrative-share access on $share"
      fi
    done
  done
else
