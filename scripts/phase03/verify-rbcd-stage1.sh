#!/usr/bin/env bash
# Verify RBCD Stage 1: candidate exists, RBCD is still untouched.
set -euo pipefail
ROOT="${ROOT:-$HOME/Documents/GOAD_NOMAD}"
PLAYBOOK="$ROOT/ansible/phase03-rbcd-verify-stage1.yml"
...
"$ANSIBLE_PLAYBOOK" -i "$DATA_INVENTORY" -i "$PROVIDER_INVENTORY" "$PLAYBOOK"
